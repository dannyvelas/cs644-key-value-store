## behavior
- [x] add compact function
- [x] make compact function get called by signal
- [x] print process id at start of session for user
- [x] compact doesn't seem to compact enough
- [x] (can't) add support for readline bindings to netcat like Ctrl+U, Ctrl+k, Alt+d, Ctrl+w
- [x] show list of supported commands in welcome message
- [x] make code send itself signal instead of calling `handler.handle("compact")`

## correctness
- [x] make server handle Ctrl+C gracefully
- [x] allow use to exit session by Ctrl+d or "exit" command
- [x] make sure signal handling is graceful
- [x] close epoll?
- [x] remove all the print statements
- [x] make sure error handling like no key found is graceful

## cosmetic
- [x] probably encapsulate file reading in its own class
- [x] make sure that all cases of `libc::write` use the `safe_` equivalents in server.rs
- [x] make sure that all cases of `libc::read` use the `safe_` equivalents in server.rs
- [x] make signal logic cleaner in `main.rs`
- [x] remove duplication of logic in disk/reader.rs when reading the key-value sizes
- [x] remove duplication of logic in `append_key` in disk/map.rs when creating key-value pairs
- [x] add Entry::from_bytes function
- [x] remove duplication of logic in disk/map.rs of pattern of open+lock+someop+unlock

## subjective
- [x] maybe make the help message be generated by handler and prettified by server
- [x] make key-value slots 2 bytes instead of 4
- [ ] use macro for error handling
